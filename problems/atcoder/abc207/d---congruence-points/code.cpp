...
bool deq(double a, double b) {
    ...
    for (int n = -3; n <= 3; n++)
        if (abs(a - b + TAU * n) <= 1e-9)
            return true;
    return false;
}

double angle_from_to(P a, P b) {
    double da = angle(a), db = angle(b);
    return da - db;
}
...
int main() {
    ...
    for (int c = 0; c < N; c++) {
        // put S[0] and T[c] in the origin
        auto sdelta = S[0];
        auto tdelta = T[c];
        vector<P> aroundS, aroundT;
        for (int i = 0; i < N; i++) {
            S[i] -= sdelta;
            if (i != 0)
                aroundS.push_back(S[i]);
        }
        for (int j = 0; j < N; j++) {
            T[j] -= tdelta;
            if (j != c)
                aroundT.push_back(T[j]);
        }
        // is there a rotation of S that maps to T?
        sort(begin(aroundS), end(aroundS), [&](auto a, auto b) {
            double da = angle(a), db = angle(b);
            return deq(da, db) ? a.norm2() < b.norm2() : da < db;
        });
        sort(begin(aroundT), end(aroundT), [&](auto a, auto b) {
            double da = angle(a), db = angle(b);
            return deq(da, db) ? a.norm2() < b.norm2() : da < db;
        });
        // try to match aroundS[i..] with aroundT[i+o..]
        for (int o = 0; o < N - 1; o++) {
            bool ok = true;
            for (int i = 0; i < N - 1 && ok; i++) {
                ok = aroundS[i].norm2() == aroundT[(i + o) % (N - 1)].norm2();
            }
            if (ok) {
                double a = angle_from_to(aroundS[0], aroundT[o]);
                for (int i = 1; i < N - 1 && ok; i++) {
                    ok = deq(a, angle_from_to(aroundS[i], aroundT[(i + o) % (N - 1)]));
                }
                if (ok) {
                    cout << "Yes\n";
                    return 0;
