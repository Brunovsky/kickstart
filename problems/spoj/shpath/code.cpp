...
auto dijkstra(int s, int t, const vector<vector<pair<int, Cost>>>& adj) {
...
auto solve() {
    ...
    for (int u = 0; u < N; u++) {
        ...
        int neighbors;
        cin >> name >> neighbors;
        idmap.emplace(name, idmap.size());
        adj[u].reserve(neighbors);
    ...
    for (int q = 0; q < Q; q++) {
        string sname, tname;
        cin >> sname >> tname;
        int s = idmap.at(sname);
        int t = idmap.at(tname);
        cout << dijkstra(s, t, adj) << endl;
    }
}
