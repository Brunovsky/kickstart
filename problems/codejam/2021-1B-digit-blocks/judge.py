...
def GetBoundary(p, q):
    ...
    return (NUM_CASES * p * S_NUMERATOR + q * S_DENOMINATOR -
            1) // (q * S_DENOMINATOR)
...
TOO_SMALL_SUM_ERROR = "Too small sum: {} | Range: {}.".format
...
def RunCases(num_cases, num_towers, height, need_correct):
    ...
    print("Got: {} ({}%)".format(total_score, total_score / need_correct),
          file=sys.stderr)

    if total_score < need_correct:
        Output(TOO_SMALL)
        raise Error(
            TOO_SMALL_SUM_ERROR(total_score, total_score / need_correct))
    Output(ALL_CORRECT)
