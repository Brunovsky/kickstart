...
struct mincost_edmonds_karp {
    ...
    void reprice() {
        for (int u = 0; u < V; u++) { pi[u] = min(dist[u] + pi[u], costsuminf); }
    }
    ...
    pair<FlowSum, CostSum> mincost_flow(int s, int t, FlowSum F = flowsuminf) {
        ...
        while (sflow < F && dijkstra(s, t)) {
            ...
            for (int e : augmenting_path) { df = min(df, edge[e].cap - edge[e].flow); }
            sflow += df;
    ...
    void clear_flow() {
        for (int e = 0; e < E; e++) { edge[e].flow = 0; }
    }
...
int main() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        if (u != v) { mcmf.add(u, v, cap, cost); }
    }
    auto [flow, cost] = mcmf.mincost_flow(0, N - 1, K);
    if (flow == K) {
        cout << cost << endl;
    } else {
        cout << -1 << endl;
