...
mt19937 rng(default_random_device());
...
void shuffle(RandomIt first, RandomIt last, URBG &&g = rng) {
  ...
  typedef uniform_int_distribution<diff_t> distr_t;
...
template <typename T>
T random_int(T min = 0, T max = numeric_limits<T>::max()) {
