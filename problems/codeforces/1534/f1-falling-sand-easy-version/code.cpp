...
int main() {
    ...
    vector<int> leftmost(C, M - 1), rightmost(C, 0);
    for (int u = 0; u < V; u++) {
        int c = cmap[u];
        leftmost[c] = min(leftmost[c], where[u][1]);
        rightmost[c] = max(rightmost[c], where[u][1]);
    }

    sort(begin(need), end(need), [&](int u, int v) {
        return make_pair(leftmost[u], u) < make_pair(leftmost[v], v);
    });
    for (int i = 0; i + 1 < K; i++) {
        assert(rightmost[need[i]] < leftmost[need[i + 1]]);
    }
