...
auto solve() {
    ...
    vector<bool> ok(S - T + 1, true);
    for (char c : {'A', 'G', 'T', 'C'}) {
        vector<int> far(S);
        int L = -k, R = k, window = 0;
        for (int i = 0; i <= min(R, S - 1); i++) {
            window += s[i] == c;
        }
        far[0] = window == 0;
        for (int i = 1; i < S; i++) {
        ...
        reverse(begin(far), end(far));
        auto mul = fft::fft_multiply(far, text);

        for (int i = 0; i < S - T + 1; i++) {
            ok[i] = ok[i] && mul[S - 1 - i] == 0;
        }
    }
    int oks = count(begin(ok), end(ok), true);
    return oks;
}
