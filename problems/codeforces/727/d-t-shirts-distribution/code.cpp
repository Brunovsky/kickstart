...
// LOL! Totally not overkill. Nice exercise though.
// Usability of simplex is a bit low :/
...
auto solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        auto j = line.find(',');
        if (j == string::npos) {
            idof[i] = find(names, names + SZ, line) - names;
        } else {
    ...
    for (int i = 0; i < N; i++) {
        if (idof[i] < 6) {
            cout << names[idof[i]] << endl;
        } else {
            int a = idof[i] - 6, b = a + 1;
            if (each[a][0]) {
                each[a][0]--;
                cout << names[a] << endl;
            } else {
