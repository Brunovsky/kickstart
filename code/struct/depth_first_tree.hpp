...
struct depth_first_tree {
    ...
    struct access_manager {
      private:
        friend depth_first_tree;
        depth_first_tree* dft;
        int u, l, r;
        access_manager(depth_first_tree* dft, int u) : dft(dft), u(u) {
            tie(l, r) = dft->splice(u);
        }
        access_manager(const access_manager&) = delete;
        access_manager(access_manager&&) = delete;

      public:
        DFTNode* operator->() { return &dft->t[first(u)].node; }
        ~access_manager() noexcept { dft->splay_join(l, first(u), r); }
    };

    vector<Node> t;

    explicit depth_first_tree(int N = 0) : t(2 * N + 1) {
        t[0].splay_size = 0;
        for (int u = 1; u <= N; u++)
  ...
  public:
    ...
    DFTNode* access_node(int u) { return splay(first(u)), &t[first(u)].node; }
