...
bool verify_edges_undirected(const edges_t& g, int V, int E = -1) {
    if (E != -1 && int(g.size()) != E)
        return false;
    unordered_set<array<int, 2>> edgeset;
    for (auto [u, v] : g) {
...
bool verify_edges_undirected(const edges_t& g, int V, vector<int> D) {
    assert(V == int(D.size()));
    int E = g.size();
...
bool verify_edges_directed(const edges_t& g, int V, vector<int> I, vector<int> O) {
...
bool verify_edges_bipartite(const edges_t& g, int U, int V, vector<int> A,
                            vector<int> B) {
...
void add_parent_edges(edges_t& g, const vector<int>& par, int u1, int u2,
...
void add_edges_except_undirected(edges_t& g, const edges_t& h, const vector<int>& par,
                                 int S) {
    for (auto [u, v] : h) {
        if (S == 0)
            break;
...
void add_edges_except_directed(edges_t& g, const edges_t& edges, const vector<int>& par,
                               int S) {
    for (auto [u, v] : edges) {
        if (S == 0)
            break;
...
void add_level_back_edges(edges_t& g, int u1, int u2, double q, const vector<int>& R) {
    boold distq(q);
...
void complete_levels_undirected(edges_t& g, const vector<int>& R) {
    int start = 0, ranks = R.size();
    for (int r = 0; r < ranks; r++) {
...
void complete_levels_directed(edges_t& g, const vector<int>& R) {
    int start = 0, ranks = R.size();
    for (int r = 0; r < ranks; r++) {
...
auto complete_multipartite(const vector<int>& R) {
...
auto circulant(int V, const vector<int>& offsets) {
    edges_t g;
    for (int o : offsets)
