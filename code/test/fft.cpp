...
template <typename I>
auto naive_multiply(const vector<I>& a, const vector<I>& b) {
...
template <typename I>
auto naive_square(const vector<I>& a) {
...
void stress_test_fft_multiply(int N = 1000, int V = 1000) {
    ...
    LOOP_FOR_DURATION_OR_RUNS_TRACKED(4000ms, now, 100'000, runs) {
        print_time(now, 4000ms, 40ms, "stress test fft multiply");
...
void stress_test_fft_square(int N = 1000, int V = 1000) {
    ...
    LOOP_FOR_DURATION_OR_RUNS_TRACKED(4000ms, now, 100'000, runs) {
        print_time(now, 4000ms, 40ms, "stress test fft square");
...
inline namespace unit_testing_fft {

void unit_test_fft_reverse() {
    for (int n = 5; n >= 1; n--) {
        auto vec = fft::fft_reverse_cache::get(n);
        print("rev[{}]: ({})\n", n, vec);
    }
}

void unit_test_fft_roots() {
    print("roots: {}\n", 4, fft::fft_cache<complex<double>>::get_roots(4));
}

} // namespace unit_testing_fft
